...
import { getAllAnnouncementsAdmin } from "@/lib/admin/queries";
  ...
  return (
    <Container className="max-w-none px-6 py-10 lg:px-10">
      <div className="flex items-center justify-between">
        <h1 className="font-display text-2xl text-parish-900">Pengumuman</h1>
        <Button href="/admin/pengumuman/baru" size="sm">
          <Plus size={15} />
          Pengumuman Baru
        </Button>
      </div>
          ...
          columns={[
            {
              ...
              cell: (a) => (
                <div>
                  ...
                  {a.is_priority && <span className="text-xs text-gold-600">Penting</span>}
                </div>
              ),
            },
            { header: "Kategori", cell: (a) => <Badge>{a.category}</Badge> },
            {
              header: "Status",
              cell: (a) => (
